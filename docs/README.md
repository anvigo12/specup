...
| **[Research: agent stack](research/specup-agent-stack.md)** | The twelve components in `open-swe/`, the seven-stage retrieval pipeline, where the licence risk sits now that SpecUP composes this stack rather than shipping it, and the stack scored against the fifteen factors. **Research, plus ten decisions taken — none of them yet an ADR.** The eighteen questions are answered from established practice and reopen as each component matures |
