# Agent Operating Contract — My Program
...
>
> This copy is the shipped template with one section added at the end. Nothing in the template
> was removed: a project that waters down its own operating contract has kept the file and lost
> the thing the file was for.
...
There is **no `src/` in this tree**, and no `src/AGENTS.md` either. My Program governs a
deliverable that lives in a different root (`my-project/`), so the traceability perimeter here
is empty and every rule above about changing code applies in that other repository rather than
this one. Read `traceability.perimeter` in `openup-config.yml` before assuming a coverage
figure from this tree says anything about code.

**Do not put a bare check id in an `index.md`.** `CTX-002` reads any id-shaped token in a
context map as an artifact reference, and `WBS-004` and `RISK-005` are id-shaped. They are
check ids, they resolve to nothing, and the check correctly reports them as dangling. Name the
check by what it does, or quote it somewhere that is not a context map. The same collision does
not arise for `TRC-`, `DOC-`, `CTX-` or `APV-`, which share no prefix with an artifact type.
