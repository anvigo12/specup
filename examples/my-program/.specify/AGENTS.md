# Agent Operating Contract — `.specify/` in My Program
...
`derived.yaml` in this tree is not empty, and it is worth knowing why. The
`wbs-iteration-field` rule reads each WBS node's `iteration:` field and produces a
`belongs-to` edge, so derivation fires here even though there is no code to scan — ten of the
fourteen edges in this graph are machine-derived and `TRC-010` reproduces every one of them.

The other four are `asserted`, and that is the correct label. Relabelling one `derived` would
not raise the provenance mix; `TRC-010` re-runs the rule, finds the rule does not produce that
edge, and fails the graph.
