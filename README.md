...
| `evaluate_gate.py` | 23 conditions | every condition name declared in config |
...
- **An approval binds a human only when it is witnessed.** Since 0.1.2 an approval has two
  levels, and the difference is the whole of what can be claimed. **Witnessed**: the approval
  names a commit, the commit exists, and its signature verifies against
  `.specify/governance/allowed-signers` — a trust root kept in version control, so the answer
  does not depend on the machine the check runs on (`APV-001`, `APV-002`). **Claimed**: a name
  and a date, and nothing tying them to a person — `approve_edge.py --by product-owner` is
  still something an agent can run, and a claimed approval proves exactly as much as it did in
  0.1.1. `APV-004` warns on every claimed approval and the audit prints the witnessed/claimed
  split, so the gap is visible rather than assumed away; `approvals.require_witness_at_or_above`
  is the ratchet that makes it fail. Two further limits stand: the trust root is only as good
  as the keys listed in it, and `approved_endpoints_hash` still proves only that an edge
  matches what was signed off (`TRC-013`), not that the signer read it.
